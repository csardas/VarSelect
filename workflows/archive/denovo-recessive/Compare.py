    ...
    for a in affected_indexes:
        ...
        for line in f4:
            if line.startswith('#'):
                ...
                chrom = tmp[0]
                pos = tmp[1]
                FC_genotype = tmp[FC_index].split(':')[0]

                genotype = tmp[a].split(':')[0]
                if chrom+'-'+pos in data:
                    # must be the same as First Child
                    if FC_genotype in REF and genotype not in REF:
#                        del data[chrom+'-'+pos]
                        removed_count += 1
                        removed_variants[chrom+'-'+pos] = genotype
#                        print ('[Removing] Chrom: ' + chrom + '\tPos: ' + pos)
                    elif FC_genotype in ALT and genotype not in ALT:
#                        del data[chrom+'-'+pos]
                        removed_count += 1
                        removed_variants[chrom+'-'+pos] = genotype
#                        print ('[Removing] Chrom: ' + chrom + '\tPos: ' + pos)
                    else:
                        # change data value to new child genotype
    ...
    for u in unaffected_indexes:
        ...
        for line in f4:
            if line.startswith('#'):
                ...
                if chrom+'-'+pos in data:
                    # can't be the same as First Child
                    if FC_genotype in REF and genotype in REF:
#                        del data[chrom+'-'+pos]
                        removed_count += 1
                        removed_variants[chrom+'-'+pos] = genotype
#                        print ('[Removing] Chrom: ' + chrom + '\tPos: ' + pos)
                    elif FC_genotype in ALT and genotype in ALT:
#                        del data[chrom+'-'+pos]
                        removed_count += 1
                        removed_variants[chrom+'-'+pos] = genotype
#                        print ('[Removing] Chrom: ' + chrom + '\tPos: ' + pos)
