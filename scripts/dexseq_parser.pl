...
use lib "/peterhouse/lchen/csardas/PerlLib" ;
use csaParseBio::Vcf_parser ;
...
my $file_ped = $opts->{p} ;
my $dir_log = $opts->{l} || "xprtest" ;
my $dir_work = "$dir_log/xpr" ;
my $file_output = $opts->{o} || $input_path . $input_name . "\_xpr.vcf.gz" ;
my $threads_active = 16 ;


`mkdir $dir_log` unless (-e $dir_log) ;
`mkdir $dir_work` unless (-e $dir_work) ;
my $file_log = "$dir_log/running_xpr_$jobid.log" ;
...
# zzz
my $file_gff_dexseq = "/peterhouse/tools/bcbio/genomes/Hsapiens/GRCh37/rnaseq/ref-transcripts.dexseq.gff3" ;
tlog("Loadig $file_gff_dexseq start") ;
...
# step 0-2 gtf
# zzz
my $file_gtf = "/peterhouse/tools/bcbio/genomes/Hsapiens/GRCh37/rnaseq/ref-transcripts.gtf" ;
tlog("Loading $file_gtf start") ;
my $genepos = {} ;
...
while (my $line = <$SRC>) {
    chomp $line ;
    my ($sample,$file) = split /\,/ , $line ;
    if ($file =~ /counts$/) {
	$sample2file->{$sample}->{rc} = $file ;
    } elsif ($file =~ /dexseq$/) {
	$sample2file->{$sample}->{exon} = $file ;
    } elsif ($file =~ /fpkm$/) {
	$sample2file->{$sample}->{fpkm} = $file ;
    } elsif ($file =~ /tpm$/) {
	$sample2file->{$sample}->{tpm} = $file ;
    } else {
	die ("wrong data type in expression file list, please check it!\nOnly support .counts .dexseq .fpkm .tpm from dexseq\n" ) ;
    }
}
close $SRC ;
